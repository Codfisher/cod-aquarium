...
2. **收集審查材料**：
   - 程式碼：`git log --oneline` + `git diff --stat` 取得變更概觀，列出關鍵檔案路徑。
...
3. **依類別設計兩個代理人的視角**（原則：兩個代理人要**正交互補**，不是重疊）：
   ...
   | 類別 | 代理人 A | 代理人 B |
   |------|---------|---------|
   | 程式碼變更 | 正確性審查：驗證修復是否到位、邊界、有無新問題、API 對齊 | 測試覆蓋率：實測 coverage、找 0% 方法、測試品質 |
...
4. **並行啟動兩個 `Explore` 代理人**（同一個 tool-use 訊息發兩個 `Agent` 呼叫 + `run_in_background: true`）。每個 prompt 應包含：
   ...
   Prompt 預期 400–600 字以上。越精確產出越可用。
...
6. **整合報告**（由 Claude 自己做，不要再開第三個彙整代理人）：
   ...
   - 將所有 action 分成三層並給明確位置：
     ...
     - **延後** — 另開 issue / ticket 追蹤的大型改進（IOptions、logging framework、設計重構）
   - 每項附預估工時（「30 分鐘」「1 小時」「需 1 人日」）讓使用者可以排程。

7. **請使用者決策**：提供明確選項（全做 / 只做必修 / 補某類別 / 僅報告）讓使用者挑，**不要擅自動手**。

## 注意事項

- **兩個代理人都用 `Explore` subagent_type**（研究取向，不改檔）；不要用 general-purpose。
...
- **前後端 / 跨 repo 情境**：在 prompt 中明確告知兄弟目錄路徑（例如 `../alvin-tccf-front-2026/`），讓代理人能搜相依。
