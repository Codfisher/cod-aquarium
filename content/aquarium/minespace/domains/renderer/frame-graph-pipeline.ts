...
export async function createFrameGraphPipeline({
   ...
   * 主場景這一關不要做影像處理
   *
   * 預設是 false，也就是「材質畫的時候就順便套上場景的影像處理設定」。
   * 但鏈的最後還有一個 imageProcessingTask，兩邊都做就是套兩次——
   * 色調映射被壓兩遍、暗角疊兩層
   */
  mainTask.disableImageProcessing = true
   ...
   *
   * 它讀的是場景的 imageProcessingConfiguration，
   * 也就是日夜循環每一幀在寫的那一份，所以不必另外接線
   */
  const imageProcessingTask = new FrameGraphImageProcessingTask('image-processing', frameGraph)
