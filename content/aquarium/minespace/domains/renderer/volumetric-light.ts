 ...
 * 不管亮不亮，天空至少先吃到幾成的保護
 *
 * 天空那道保護原本整份跟著亮度走，而那條路上藏著一個回授圈：
 * brightRatio 讀的是已經合成好的畫面亮度，那份亮度裡含著太陽的光暈，
 * 於是光暈每加一分亮度，headroom 就扣掉一分光束。
 *
 * 圈的增益是「相位 × 強度 × 亮度曲線的斜率」，整份跟著亮度走時
 * 大約是一點九——大於一的意思是，光暈每加一分，畫面反而暗零點九分。
 * 光暈從亮到零的路上必然穿過那一段，穿過的那一圈就凹下去，
 * 露出沒被光束洗白的原始天色：太陽外圍因此鑲了一道又藍又暗的邊。
 *
 * 這件事調光暈救不了。加強光暈會等比放大回授的兩邊，增益不動，
 * 那圈凹陷一階都不會變淺，只會往外移——量過，一階都沒動。
 *
 * 這個底把跟著亮度走的部分縮到三成五，增益因此掉到一以下，
 * 剖面是單調的。順帶也修好另一件事：正午藍天的亮度只有零點五三，
 * 原本 brightRatio 只剩零點一，SKY_FLOOR 幾乎沒生效，
 * 整片天被加上將近半個單位的暖光，太陽整個糊在裡面看不清楚。
 *
 * 往一調是「天空一律保護，光束完全不上天」，往零調則是原本那套。
 * 這個值就是「天要白到什麼程度」的旋鈕
 */
const float SKY_GUARD_BASE = 0.65;
...
void main() {
  ...
  float skyRatio = farRatio * mix(SKY_GUARD_BASE, 1.0, brightRatio);
