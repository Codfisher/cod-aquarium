...
export function useWeather() {
  ...
  async function start({ scene, camera, isSheltered, getGroundY }: StartParams): Promise<void> {
    if (rainParticles)
      return

    rainParticles = createRainParticles(scene, quality.value === 'low' ? 1200 : 3600)
    ...
    scene.onBeforeRenderObservable.add(() => {
      ...
      if (flashTimeLeft > 0) {
        ...
        atmosphere.flashRatio = flashTimeLeft / 0.35
      }
      else {
      ...
      if (skyMaterial) {
        skyMaterial.luminance = lerp(0.55, 0.25, rainRatio)
        skyMaterial.turbidity = lerp(6, 22, rainRatio)
        skyMaterial.rayleigh = lerp(1.6, 0.5, rainRatio)
      }
      ...
      rainParticles?.update(camera, isSheltered() ? 0 : rainRatio, getGroundY())
