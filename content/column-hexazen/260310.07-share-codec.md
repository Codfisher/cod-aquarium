...
HexaZen 的設計理念之一是「分享連結即可讓他人聽到你設計的自然音效」。

這代表場景資料要塞進 URL 裡，不能靠後端資料庫。

JSON 太長了，Base64 編碼 JSON 也不夠精簡。所以我們需要自己設計位元打包（Bit-packing）格式。<span class="text-nowrap">%੭ ˙ᗜ˙ )੭%</span>
...
座標使用偏移量技巧：`q + 3` 把 [-3, 3] 轉成 [0, 6]，剛好可以用 3 bits 表示。
...
標準 Base64 的 `+`、`/`、`=` 在 URL 中有特殊含義，替換成 `-`、`_` 並去掉 padding。<span class="text-nowrap">( •̀ ω •́ )✧</span>
...
function decodeBinaryBlocks(encoded: string): SharedBlock[] {
  ...
  for (let i = 0; i < blockCount; i++) {
    const char1
      = String.fromCharCode(reader.read(BITS_CHAR))
    const char2
      = String.fromCharCode(reader.read(BITS_CHAR))
...
async function handleShare() {
  const sharedBlocks = Array.from(
    placedBlockMap.values(),
  ).map((block) => {
    const rotationStep = Math.round(
      block.rootNode.rotation.y / (Math.PI / 3),
    )
...
旋轉角度從弧度轉回步數（0～5），確保 `((step % 6) + 6) % 6` 處理負數旋轉。

### 還原：載入分享連結

```typescript
async function restoreSharedView() {
  if (!sharedViewEncodedData) return
  ...
  try {
    ...
    const tasks = sharedBlocks.map(({ type, hex, rotation }) => {
      return mainSceneRef.value!.spawnBlock(type, hex)
        .then((block) => {
    ...
    await promiseTimeout(
      Math.max(3000, new Date().getTime() - startedAt),
    )
...
還原後的檢視模式會自動開啟攝影機自動旋轉，讓使用者可以悠閒地欣賞音景。<span class="text-nowrap">%✧⁑｡٩(ˊᗜˋ*)و✧⁕｡%</span>
...
壓縮率超過 90%！URL 短而美觀。<span class="text-nowrap">(/≧▽≦)/</span>
...
下一章讓我們來看 UI 設計與國際化的實作吧。<span class="text-nowrap">♪( ◜ω◝و(و</span>
