...
JSON 太長了，Base64 編碼 JSON 也不夠精簡。所以我們需要自己設計位元打包（Bit-packing）格式。%੭ ˙ᗜ˙ )੭%
...
標準 Base64 的 `+`、`/`、`=` 在 URL 中有特殊含義，替換成 `-`、`_` 並去掉 padding。%( •̀ ω •́ )✧%
...
還原後的檢視模式會自動開啟攝影機自動旋轉，讓使用者可以悠閒地欣賞音景。%✧⁑｡٩(ˊᗜˋ*)و✧⁕｡%
