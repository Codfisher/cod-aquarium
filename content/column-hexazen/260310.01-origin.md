...
<script setup> // [!code ++]
import Hexazen from './hexazen.vue' // [!code ++]
</script> // [!code ++]

<iframe data-why class="fixed w-dvw h-dvh"> // [!code ++]
  <hexazen /> // [!code ++]
</iframe> // [!code ++]
```
...
以 domain 為單位組織程式碼，每個 domain 自成一體。
...
下一章讓我們來看六角格座標系統與聲景規則的設計吧。<span class="text-nowrap">♪( ◜ω◝و(و</span>
