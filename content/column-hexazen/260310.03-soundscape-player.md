...
但是「播放音效」本身其實也沒那麼簡單。<span class="text-nowrap">%(›´ω`‹ )%</span>
...
export type SoundscapeType = 'rustle' |
  'insect' | 'bird' | 'frog' | 'beast' |
  'river' | 'building' | 'ocean' |
  'alpine' | 'rain' | 'campfire'
...
`OVERLAP_SECONDS` 預設為 4 秒，但不會超過音檔長度的 40%，避免短音檔出問題。<span class="text-nowrap">( •̀ ω •́ )✧</span>
...
三步驟：停止排程 → 漸出音量 → 徹底清理資源。乾淨利落。<span class="text-nowrap">%✧⁑｡٩(ˊᗜˋ*)و✧⁕｡%</span>
...
export function useSoundscapePlayer(
  ...
  const activePlayerMap = shallowReactive(
    new Map<number, SoundscapePlayer>(),
  )
...
下一章讓我們來看看 3D 場景中的互動機制吧。<span class="text-nowrap">♪( ◜ω◝و(و</span>
