...
沒關係，那就自己做一個。<span class="text-nowrap">ᕕ( ﾟ ∀。)ᕗ</span>
...
用 `useElementSize` 動態取得按鈕高度，讓倒角比例自動適配不同大小的按鈕。<span class="text-nowrap">( •̀ ω •́ )✧</span>
...
只顯示目前正在播放（未被靜音）的音效，避免混亂。<span class="text-nowrap">%(\*´∀`)~♥%</span>
...
因為 whyframe 環境的限制，我們自己做了一個輕量版的 i18n composable。
...
如果中文多了一個 key 但英文沒有，TypeScript 編譯時就會報錯。不用等到運行時才發現遺漏翻譯。<span class="text-nowrap">%✧⁑｡٩(ˊᗜˋ\*)و✧⁕｡%</span>
...
左側控制音效相關，右側控制積木操作。積木選擇器從底部滑出。

所有按鈕都加上 `drop-shadow` 白色光暈，確保在任何背景色上都清晰可見：
...
下一章是最後一篇，讓我們來聊聊 WebGPU 與效能優化。<span class="text-nowrap">♪( ◜ω◝و(و</span>
