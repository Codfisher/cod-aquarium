...
HexaZen 有 30 多種積木，使用者需要一個直覺的方式來挑選。

光靠文字名稱太無趣了，我們需要每個積木的 3D 預覽縮圖。

但 30 多張圖片手動截圖太累了，而且每次改積木就要重做。<span class="text-nowrap">%(›´ω`‹ )%</span>

解決方案：用無頭的 Babylon.js 場景自動產生縮圖！

## 多執行緒縮圖生成器

核心概念是建立多個隱藏的 canvas，每個 canvas 都有獨立的 Babylon.js 引擎，像工人一樣平行生成縮圖。
...
流程：載入模型 → 自動框住 → 渲染 → 截圖 → 轉 Blob。

每個工人一次只處理一個任務（`concurrency: 1`），但多個工人可以同時工作。
...
`drag-free` 讓使用者可以自由拖拽瀏覽，手機上也很順暢。<span class="text-nowrap">%◝(≧∀≦)◟%</span>
...
不管幾個元件呼叫 `useThumbnailGenerator()`，都會拿到同一個實例，共享同一批工人。
...
下一章讓我們來看分享編碼與 URL 壓縮的實作吧。<span class="text-nowrap">♪( ◜ω◝و(و</span>
