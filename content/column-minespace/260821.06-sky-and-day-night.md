...
先講最要緊的一條，因為雲的形狀怎麼長都得先服從它。

```ts
const patternCount = Math.round(WORLD_SIZE / CLOUD_CELL_SIZE)
```

雲的圖樣必須剛好一個世界寬。玩家跨過邊界時位置會平移一整個世界的距離，圖樣若不是同一個週期，整片天會在那一瞬間換一副樣子。

而雲是全場唯一不吃霧氣、藏不住的東西。%( ˘•ω•˘ )%

這裡原本寫死 18，格寬 14，乘起來 252。而世界寬度是 280，這兩個數字其實從來沒對上過。%(´థ‸థ)%

除出來就不會再有下一次。

### 用可平鋪的噪音長出雲

週期定死之後，剩下的問題是怎麼在這 20 × 20 格裡長出一朵一朵的雲。

一開始用的是細胞自動機，灑一把亂數，再跑幾輪「鄰居滿五格就是雲」的平滑規則。

這招長洞穴很好用，長雲卻踩到一個坑。%(›´ω`‹ )%

那組規則是收斂的，起始填充率不到一半時，四輪跑完只剩不到一成。後面還有一道「削掉過大的雲團」由外往內剝一圈、連剝六輪，一大塊會被剝到只剩正中心那幾格。

兩道疊起來，全世界只剩十二格雲。

而且那十二格全擠在同一條 Z 帶上。雲只往 +X 飄，那條帶以外的地面，從開場到關掉分頁都不會有雲經過。%(´థ‸థ)%
...
門檻取分位數而不是給一個定值，這樣覆蓋率是指定的而不是算出來的。說要蓋掉一成四的天空，跑出來就真的是一成四。%(๑•̀ㅂ•́)و✧%

低頻那一層決定一朵雲多大，高頻那一層權重只有三分之一，負責在邊緣咬出缺口。少了它每朵雲都圓滾滾的，看起來像水滴不像雲。

噪音本身也得能平鋪。simplex 那類函式沒有週期，接縫上兩側的值對不起來，所以自己灑格點再平滑內插。

```ts
function readPoint(pointList: number[], pointCount: number, x: number, z: number) {
  /** 座標繞回去，放大出來就是無縫的 */
...
覆蓋率拉到 0.45 就是連成一大塊蓋住整片天，拉到 0.05 則是一抬頭什麼都沒有；細節權重歸零，每朵雲都變回圓滾滾的一團。

順帶一提，這個數字的下限不是由「好不好看」決定的。雲只往 +X 飄，覆蓋率一低就會出現整條 Z 列從頭到尾都沒有雲經過，那條線上的地面於是永遠曬不到雲影——那正是 EP10 那樁懸案的兇手。
...
- 雲用可平鋪的噪音長出來，圖樣週期必須等於世界寬度，還要做面剔除與距離淡出
