...
`vec4(x, y, z, w)` 其實是數學上的**齊次座標**（homogeneous coordinates），多出來的 w 讓平移、旋轉、縮放、投影都能用同一套矩陣乘法搞定。
...
::: tip 其實不是單純的線性插值
預設的插值模式是**透視校正插值**（perspective-correct interpolation），會把頂點的 w 分量納入計算，確保 3D 透視場景下的插值結果看起來正確。
