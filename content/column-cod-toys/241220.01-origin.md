...
- PowerToys
  ...
  其中的 [PowerToys Run](https://learn.microsoft.com/zh-tw/windows/powertoys/run) 工具是我們的主要靈感來源，~~抄襲~~借鑑其設計。
...
介面則模仿 [PowerToys Run](https://learn.microsoft.com/zh-tw/windows/powertoys/run) 的啟動器，按下 `Ctrl + Space` 就會跳出如下圖輸入框。
...
- [unplugin-vue-router](https://github.com/posva/unplugin-vue-router)（不重要）

:::tip
此專案用不到 router，不過懶得從模板中移除惹。<span class="text-nowrap">(´,,•ω•,,)</span>

`unplugin-vue-router` 真的很讚，有機會來寫一篇文章介紹一下。
:::
