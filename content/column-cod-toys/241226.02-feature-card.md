...
由於視窗由 main process 控制，我們需要讓網頁可以透過 `ipcRenderer` 來通知 main process 調整視窗大小。
...
`preload.ts` 中的 `contextBridge` 模組會將 `main` 物件注入在 window 中，我們可以在網頁中使用 `window.main` 來呼叫 main process 的方法。

這裡我們把 `mainApi` 透過 Composition API 形式提供，保留未來更動彈性。
...
現在讓視窗根據內容高度自動調整吧！ヽ(●`∀´●)ﾉ

使用 `useElementBounding` 輕鬆完成。
...
最後讓我們在 `main-input` 中註冊鍵盤事件，來選擇選項。
...
不過每次新增功能卡片都要手動引入好麻煩喔，讓我們自動引入吧！<span class="text-nowrap">( ´ ▽ ` )ﾉ</span>
