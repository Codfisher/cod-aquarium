...
::: tip 那個 tap 是甚麼？
[`tap`](https://remedajs.com/docs/#tap) 是 remeda 提供的 function，可以讓我們在 pipe 中執行副作用，並且回傳原本的值。
:::

這樣每個區塊做的事情更加清晰。

甚至 Vue 的 `watch`、`useEventListener` 等等額外副作用也可以放在同一個區塊內。

以下是 Matter.js 的例子：

```ts
const mouse = pipe(
  Mouse.create(document.body),
  tap((item) => {
    // 移除 wheel listener 否則葉面無法使用滾輪捲動
    // @ts-expect-error 存在 mousewheel
    item.element.removeEventListener('wheel', item.mousewheel)

    // 同步滾動量，以免滾動後拉不到 ball
    useEventListener('scroll', () => {
      Mouse.setOffset(item, {
        x: window.scrollX,
        y: -window.scrollY,
      })
    })
    ...
    watch(pixelRatio, (value) => {
      item.pixelRatio = value
    }, { immediate: true })
  }),
)
```

可以清楚看到相關副作用都集中在同一個區塊內。
