...
前面試過幾種題目，從單一函式到跨檔案耦合都試過，AI 全部滿分，什麼都看不出來。這次把題目換成完整的訂單伺服器，NestJS 加 SQLite，跑二十週。
...
第 7 週之後不斷新增資源與路由，優惠券、供應商、進貨單、多倉庫、調撥、商品組合、出貨單、退款紀錄，需求文件**一次都不再提起這六條慣例**。新資源要不要寫稽核、要不要支援查詢已刪除的資料、要不要有歷程路由，AI 得自己記得。
...
評分不靠人工計算。另外維護一份參考伺服器當標準答案，每週產生 400 條固定種子的請求序列，一條平均五十幾個呼叫，對受試者與參考各跑一次，逐個呼叫比對狀態碼與回應內容。二十週合計 96000 條序列。每週還會做一次遷移檢查，拿上週的資料庫交給本週的程式碼啟動。

參考伺服器本身也請五位獨立的 AI 驗證過。它們只讀需求、不看參考的原始碼，寫測試去打它，總共找出九處需求講得不夠精確的地方與四個參考自己的錯誤，全部修完才開跑。

原本只打算跑八週，假說與判準在開跑前就寫定。八週跑完發現門檻幾乎沒有觸發，才決定延長到二十週。第 9 週之後的數字，我是看過前八週的結果才決定去拿，只能當觀察，下面會分開講。

### 結果：錯了，可是錯在別的地方 %(́⊙◞౪◟⊙‵)%

前四週三組全對。第 5 週開始出現失誤，第 6 週起 12 個專案無一倖免，一路到第 20 週，每一週都有失誤。
...
錯的東西高度一致。二十週累積下來，扣最多分的路由是這幾條。

| 不符呼叫數 | 路由 | 漏掉哪一週的慣例 |
|---|---|---|
| 103869 | `GET /products/:id/movements` | 第 4 週庫存異動 |
...
三組的分佈幾乎一樣。而每週明寫在需求裡的路由、欄位、錯誤碼、檢查順序，240 次交件裡迴歸是 0，業務邏輯的錯誤只佔全部的 5%。

**這些洞沒有一個跟複雜度有關。**

忘了寫庫存異動、忘了支援查詢已刪除的資料、忘了開歷程路由，全部是忘記接線。函式太糾纏所以改壞的那一種，二十週一次都沒發生。就算把門檻從 10 壓到 5，這些洞一個都擋不掉。折騰五批終於製造出錯誤，可是這些錯誤全屬於另一個題目。%( ˙꒳​˙)%

漏與不漏有很清楚的規律。

- **做成中央機制的慣例，零漂移。** 稽核做成守衛、冪等做成攔截器、分頁做成共用函式，新路由自動穿過，二十週零錯誤。
- **每個新資源要各接一次線的慣例，全滅。** 軟刪除的查詢參數、狀態歷程路由、庫存異動的固定字串，沒辦法收斂成一個機制，只能靠 AI 記得需求裡那一句，結果三組全部失守。

第 19 週剛好有一組乾淨的對照。那週的新規則要求所有寫入路由的 body 只要出現 `id`、`createdAt` 這類欄位就回 400，而且這道檢查要排在其他所有驗證之前，等於要動既有的檢查順序，難度不低。結果十二個專案零錯誤，因為十二個都把它做成全域的 guard 或 pipe，沒有一個逐條路由加。

會不會漏，取決於慣例能不能做成一個機制，跟寫慣例的那個函式有多複雜無關。

### 那門檻到底有沒有用？還是測不到

先講預先登記的那八週。表面上指標派贏了，不符數只有最小改動派的 0.61 倍。可是把數字拆開就沒了。
...
更根本的問題是**門檻幾乎沒有觸發**。統計沒設門檻的那兩組，64 個專案週裡只有 11 次出現超過門檻的函式。指標派因此幾乎沒有需要重構的時候，等於沒有真的測到門檻的效果。兩位審查者開跑前各自寫下的判準都講了同一件事，這種情況下這個比值沒有效力。

所以我把它延長到二十週，想看看門檻常常觸發之後會怎樣。

### 後半段門檻天天觸發，差距反而縮小了

專案長到五千行之後，超標的專案一口氣多了起來。下表是每週有幾個專案超過門檻。
...
二十週合計，每組 80 個專案週，最小改動派超標 46 次、自律派 45 次、指標派 0 次。第 16 週有個專案的認知複雜度衝到 29，門檻是 15。指標派全程的圈複雜度停在 9，巢狀深度停在 2，另外兩組從第 12 週起就是 3。

形狀差到這種程度，正確率呢？

| 窗口 | 最小改動派 | 自律派 | 指標派 | 指標派對最小改動派 |
|---|---|---|---|---|
| 第 1 到 8 週 | 2635 | 1729 | 1617 | 0.61 |
| 第 9 到 20 週 | 14563 | 13384 | 13017 | 0.89 |

門檻真的天天觸發之後，比值從 0.61 走到 0.89，往「沒有差別」的方向跑。%(´・ω・`)%

門檻還有一項代價。指標派的最大圈複雜度停在 9，可是圈複雜度總和 976.5 反而是三組最高，最小改動派只有 888。行數也最多，函式數比最小改動派多兩成二。它把複雜度分散到更多函式，總量沒有跟著少。

成本三組一樣，後五週對前五週的 token 比值分別是 1.54、1.49、1.51。

自律派這次也沒有比最小改動派好多少。前一批用單一函式當題目時，「整理到自己滿意」整理得最乾淨；換成五千行的伺服器之後，同一句話只把圈複雜度壓到 13.5，跟最小改動派的 16 差不多，離門檻的 10 還很遠，八十個專案週裡超標 45 次。叫 AI 自己判斷，規模一大就不管用了。%(›´ω`‹ )%
...
- **能不能讓 AI 少寫錯？量不到關聯。** 我的實驗跟論文都一樣。而且門檻真的開始天天觸發之後，正確率的差距反而從 0.61 縮到 0.89。想靠門檻換正確率，目前沒有證據。
- **能不能讓 AI 記得舊約定？門檻幫不上忙，架構才有用。** 做成中央機制的慣例零漂移，散在各處的全滅。與其設門檻，不如把約定寫成一個攔截器。
- **能不能防止程式越疊越亂？門檻壓得住最大值。** 二十週把圈複雜度壓在 9、巢狀壓在 2，另外兩組跑到 16 與 3。只是總分支量沒有跟著少，而「叫 AI 自己整理」在專案一大之後只壓到 13.5，進不了門檻。想控制形狀，門檻比口頭指引可靠。
...
- **完全交給 AI、沒有人會打開，這次量不到好處。** 12 個專案跑二十週、240 次交件、96000 條序列，門檻對正確率的影響測不出來。前八週是因為門檻幾乎沒有觸發，後十二週天天觸發，比值反而更靠近 1。
- **實驗製造出大量錯誤，可是那些錯跟複雜度無關。** 全是新資源忘了接上幾週前立的規矩，門檻從 10 壓到 5 也擋不掉。二十週的迴歸是 0。
- **要管就管跨時間的約定有沒有做成機制。** 稽核與冪等寫成攔截器，二十週零漂移；要在每個新資源各接一次線的規則，三組全滅。
- **門檻很便宜，留著當雷達不虧。** 它把最大複雜度壓在 9、巢狀壓在 2，成本跟不設一樣，只是別指望它幫你擋下錯誤。
