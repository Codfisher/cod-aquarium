...
第一週，它在那個函式裡加了一個 `if`。第二週，又一個 `if`。

第三個月，打開那個函式，滾輪一路往下捲，捲到懷疑人生。%(ﾟдﾟ)%

每一次改動看起來都很合理，加起來就變成災難。

以前的函式不太會爛成這樣。同一個地方改到第五次，人自己就受不了了，會停下來整個重寫。

什麼？你說公司前人寫的祖傳程式碼都超過幾千行，早就習慣了？敬佩敬佩。%(́⊙◞౪◟⊙‵)%

其實有個 1976 年就發明出來的老指標，叫做圈複雜度（Cyclomatic Complexity）。

這篇會拿酷酷元件（[chill-component](https://chillcomponent.codlin.me/)）裡三個真實函式開刀，最後再回頭算算這筆帳。
...
鱈魚：「太好了，我要把前五名通通拆掉！%(ﾉ>ω<)ﾉ%」

路人：「先打開來看一眼吧⋯⋯」

路人說得對，接下來三個案例會得到三種完全不同的結論。%(´,,•ω•,,)%

## 案例一 · 該拆的：27 分的 watch

[btn-attention-seeker](https://chillcomponent.codlin.me/components/btn-attention-seeker/) 是一顆很愛演的按鈕，滑鼠靠近會湊過來，被視窗上下緣切掉時會探個頭出來刷存在感。
...
有趣的是，這裡只有四個 `if`。我把所有 `?.` 換成直接呼叫再量一次，只剩 5 分，也就是說 22 分全部來自 `carrierPositionAnimatable?.x?.()` 這種選擇性串連。

一開始我覺得指標在亂算，想想才發現它很誠實。`?.` 真的是分支，左邊是 `null` 就整條短路。而這串三行的動畫呼叫，在同一個函式裡抄了整整四遍，它抱怨的其實是重複。%(°ㅂ°)%

那就把重複收掉，讓每一段只負責算出目標位置，最後統一送給動畫。
...
更大的收穫在後面。那四個 `if` 原本埋在一堆動畫呼叫中間，抽出來才看清楚這個 watch 一直在做四件事，探上緣、探下緣、跟隨滑鼠、歸位。分數高，往往是因為這個函式偷偷兼了好幾份差。%(・∀・)%

## 案例二 · 該測的：17 分的輸入法地獄

[input-decoding](https://chillcomponent.codlin.me/components/input-decoding/) 是打字時字元會亂碼跳動、最後解碼成正確文字的輸入框，看起來很駭客，實作起來要跟中文輸入法搏鬥。

它的 `handleInput` 是 17 分，分支全是邊界條件，`isComposing` 拼字中、`insertText` 一般輸入、`insertFromDrop` 拖曳、`delete` 刪除，還有拼音選字造成的 caret 位置偏移。

這裡沒有重複可以收，每個 `if` 都對應一種真實存在的輸入行為。所以這個案例要看的是另外兩件事。

### 17 分就是 17 條測試的下界

圈複雜度的另一個身分，是走完所有線性獨立路徑所需要的測試數量。

這件事很容易被覆蓋率蓋過去。覆蓋率算的是「這一行、這個分支有沒有被跑過」，路徑算的是「這些分支的組合有沒有被走過」。一個 17 分的函式，三、四條測試就能讓分支覆蓋率變得很好看，離十七條獨立路徑還遠得很。

而 IME 的 bug 幾乎都藏在「拼字中又剛好按了刪除」這種組合裡。%(;´༎ຶД༎ຶ`)%

### 那排註解掉的 console.log 就是帳單
...
查一個 bug 得先印六個變數，才知道當下走在哪一條路上。這排註解就是高複雜度寄來的帳單，CI 報告上看不到，只會從下班時間扣。%(ﾟдﾟ)%
...
61 分，全專案冠軍，比第二名高出一大截。照前面的邏輯，這種分數應該當場拆成碎片才對。

可是打開來看，它是一條從頭走到尾的管線。

1. 頂點池，吸附鄰近端點
2. 初始線段，邊界加摺痕
3. 逐段切割，處理 T 形接點與線段相交
4. 去除重複邊
5. 面萃取，半邊巡走
6. 以有號面積分辨內面與外面
7. 耳切三角化
...
每一段都吃上一段的產出，中間狀態層層相依。硬要拆成九個函式，`vertexList`、`segmentList`、`snapEpsilon` 這些中間狀態就得全部拉成參數傳來傳去。讀的人得在九個地方跳來跳去，才能拼出原本一路讀下來就懂的東西。

它的職責非常單一，就是把摺線編譯成網格。61 分反映的是幾何運算本身有多繞，跟設計好壞無關。
...
看到高分要問的問題是「這個函式的職責乾不乾淨」。乾淨就放行，順手補幾行註解標出每個階段，比硬拆有用得多。
...
函式越複雜，內部共用的中間狀態就越多，改 A 分支順手弄壞 B 分支的機率也越高。

所以圈複雜度也可以當成爆炸半徑的量尺。請 AI 動一個 5 分的函式，最多影響五條路，出事了掃一眼就知道。請它動一個 61 分的，那就自己保重。%(´,,•ω•,,)%

### 三、抓得到 AI 特有的退化模式

人類工程師改同一個函式改到第五次，會開始煩躁，會忍不住整個重寫。那股煩躁其實是很寶貴的訊號，它在說這段程式的結構撐不住新需求了。

現在那個函式已經沒有人類讀者了。AI 面對新需求，最自然的反應是再塞一個 `if`，因為那是最小的 diff，看起來最安全，也最不容易打壞現有測試。每一步都是漂亮的局部最佳解，一路走進全域的爛攤子。

鱈魚：「原來 AI 這麼懶，連重寫都不肯。%( ˘•ω•˘ )%」

路人：「它超勤勞好嗎，每個需求都乖乖幫你加一個 `if`，一個都不漏。」

鱈魚：「⋯⋯這樣講怎麼更可怕了。%(°ㅂ°)%」

以前那股「這函式我看不下去了」的煩躁，要有人親手讀過才會冒出來。現在程式碼由 AI 讀、AI 改，沒人再從頭看一遍，訊號就這樣消失了。複雜度曲線剛好補上這個缺口。單看某次 PR，`+1` 分不痛不癢；把三個月的趨勢畫成一條線，它會很誠實地告訴你發生了什麼事。
...
**盯趨勢，不盯絕對值**：61 分放著沒關係，這次 PR 讓最高分從 61 變成 65 才是要追問的事。把每次掃描結果存成 JSON 丟進 CI artifact，跟上一版比一下就夠用了。

**加碼認知複雜度**：`complexity` 有個罩門，它對巢狀沒有感覺。三個平行的 `if` 跟三層巢狀的 `if` 同樣都是 4 分，可是後者難讀太多。`eslint-plugin-sonarjs` 的 `sonarjs/cognitive-complexity` 會對巢狀加權，越深扣越兇，比較貼近實際讀起來的感受。
...
- 圈複雜度就是分支路徑數，從 1 起算，每個判斷 +1，`else` 與 `default` 都不算。`?.`、`&&`、`??` 也會算，所以分數有時候高得莫名其妙，打開來才知道它在抱怨重複。
- 這個數字同時是走完所有獨立路徑所需要的測試條數，覆蓋率亮綠燈不代表路徑走完了。
- 看到高分先問職責乾不乾淨。重複造成的高分該拆，演算法本身就繞的別硬拆，`compileCreasePattern` 那 61 分我到現在還放著。
- AI 面對新需求傾向再加一個 `if`，因為那是最小 diff。以前是人讀到受不了才動手重寫，現在沒人讀了，複雜度曲線正好補上這個缺掉的訊號。
- 設 `warn` 當雷達，盯趨勢不盯絕對值，再加一條認知複雜度顧巢狀，大概就有八成的價值了。
