...
三項全紅。案例一是分數灌水，這裡反過來，61 分還不足以形容它有多難讀。%(°ㅂ°)%

打開來看，它是一條從頭走到尾的管線，頂點吸附、逐段切割、去重、面萃取、三角化、正規化，九個階段一路做到底。硬拆成九個函式，`vertexList`、`segmentList`、`snapEpsilon` 這些中間狀態就得全部變成參數傳來傳去。

管線只解釋了它不該那樣拆，沒有解釋它可以放著不管。難讀的來源是那五層巢狀，跟階段多寡無關，用 early return 把縮排拉回來就好。

那我為什麼到今天還沒動它？

因為我懶。%(´_ゝ`)%

不過它的曲線很平，這個檔案至今只有三個 commit，從第一天就是 61 分。三千行的怪物是三年來一次一個 `if` 疊出來的，那種才會愈拖愈糟。

要盯的是斜率。問題還在那裡，只是還沒輪到它。%(。-`ω´-)%

## 那分數高到底算不算有問題

三個案例看完，回頭校準一下這個數字的準度。

27 分確實灌了水，可是那 22 分背後真的躺著四份重複。17 分是真材實料，扣掉走不到的還有十一、二條要測。61 分一分都沒虛，實際讀起來還更糟。
...
實際跟 AI 協作時，這個數字最大的用處是當停止條件。直接說「幫我補測試」，它會給三、四條快樂路徑，覆蓋率變綠就收工，因為它不知道什麼叫夠。
...
有兩個地方要先講死，不然 AI 會走捷徑。

一是**先要清單，不要先要程式碼**。清單可以一條一條核對，AI 一次吐三百行程式碼，你根本不會看。

二是**允許它說做不到**。不明講的話，你要 17 條它就給你 17 條，把走不到的路徑也硬編出來。

驗收也很簡單，數清單幾列，跟分數對不上就回頭問。少了代表漏路徑，多了代表有重複。%(ゝ∀・)b%
