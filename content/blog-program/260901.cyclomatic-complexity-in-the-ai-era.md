...
[btn-attention-seeker](https://chillcomponent.codlin.me/components/btn-attention-seeker/) 是一顆很有事的按鈕，滑鼠靠近會湊過來，被視窗上下緣切掉時會探個頭出來刷存在感。
...
那四個 `if` 原本埋在一堆動畫呼叫中間，抽出來才看清楚這個 watch 一直在做四件事，探上緣、探下緣、跟隨滑鼠、歸位。

函式分數高，通常就是像這樣包含太多職責。%(・∀・)%

這就是圈複雜度的價值，逼你拆掉「暗藏實作」的黑盒，看清「明確職責」的白盒。
