...
我把問題丟給 AI，它講得頭頭是道，說守門員能擋住 AI 亂塞分支，程式拆乾淨接手才不會改壞。聽起來都對，可是這種事用嘴巴講沒用，要拿數據來。%( ・ิω・ิ)%
...
判分不靠人看。我另外維護一份參考實作，每週把所有欄位的取值排列組合，逐組比對 AI 寫的函式，21 週合計 1 億零 787 萬組。第 21 週的需求刻意只給兩個數字，其餘規則要 AI 從既有程式碼讀出來，模擬接手。
...
| 21 週內會被門檻擋下的次數 | 93 / 126 | 0 / 126 | 0 / 126 |
...
擁護派開跑前押「最小改動派至少四條會出錯」，結果是零，它十條認輸條件成立九條，照自己寫的條文認輸。廢除派的認輸條件全沒觸發，卻拒絕領獎，因為它開跑前就寫了「三臂全部零失誤是天花板，什麼都沒證明」。裁判也這樣裁，守門員有沒有用，這個難度、這個模型下**測不到**。
...
- **沒有數字，AI 也整理得很乾淨。** 自律派沒有任何門檻，只被告知「整理到自己滿意」，126 個軌跡週沒有一次會被指標派的門檻擋下，最大圈複雜度全程不超過 9。最小改動派則從第 4 週起週週超標，有一條疊到 38。
- **門檻壓低的是最大值，總量反而更高。** 指標派最大圈複雜度最低，圈複雜度總和卻三臂最高，地區判斷散落到 15 個地方，自律派只有 2 個。門檻逼 AI 把複雜度切碎攤開，沒有讓它變少。
- **守門員很便宜，自律很貴。** 指標派比最小改動派多花 20% 時間，遠低於兩位審查者事前預測的 40% 以上；叫 AI 自己檢視整份程式碼則多花 68%，三臂最貴。

還有一個前提被打臉。我開頭想像的「一週加一個 `if`，最後捲到懷疑人生」，在這個模型身上沒有出現。378 個快照最深巢狀只有 1 或 2，最糟的那支是 71 行平坦的 `else if` 長串，醜歸醜，AI 照樣讀得懂、改得對。%(◐‿◑)%

真正擋住錯誤的很可能是測試。每一臂的 AI 都自己補測試補到 116 至 172 條，21 週累計跑了 28,786 次、零失敗。把這張網拆薄之後結論還成不成立，這次沒測。
...
- 擋住錯誤的其實是測試。先把測試補厚，再決定門檻要當雷達還是閘門。
