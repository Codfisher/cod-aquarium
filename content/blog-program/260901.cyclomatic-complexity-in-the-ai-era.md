...
現在 AI 能輕輕鬆鬆讀完幾千行程式碼，這個理由其實不太成立了。
...
本篇的三個案例，我有先請 AI 算過一次，過程中它多次算錯，都是量完才校正。

它讀 code 的時候注意到的是巢狀深度、函式長度、重複的區塊，這些比較接近認知複雜度。

至於分支到底幾條，它只能靠感覺，而且沒辦法自己驗證猜得準不準。%(´_ゝ`)%

像是案例一的 27 分，22 分全部來自 `?.`，但是 AI 看的時候會被騙，誤以為短路運算子在視覺上不像分支，掃過去不會有感覺。

還有案例三，它預期扁平管線的認知複雜度應該偏低。實測 135，他直接忽略了那五層巢狀波動拳。

程式碼可以交給 AI 寫，可是它跟人一樣用感覺判斷複雜度，而感覺會漏掉短路運算子、會低估巢狀，還會幫自己找各種藉口。

當然你可以靠對抗審查等等方法，避免 AI 幫自己找藉口、亂猜，不過跑一行指令就能解決的事，何必消耗那麼多 token 呢？%(ゝ∀・)b%
...
- 想把分數變成測試清單就用基線法，先走一條主線，再回頭讓每個判斷點各翻一次。翻不動的寫下理由，硬要湊滿反而是危險訊號。
- AI 判斷複雜度也只能靠感覺，而且沒辦法驗證自己準不準。它面對新需求傾向再加一個 `if`，以前是人讀到受不了才動手重寫，現在沒人讀了，複雜度趨勢正好補上這個缺掉的訊號。
