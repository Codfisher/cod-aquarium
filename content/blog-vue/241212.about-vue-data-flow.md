...
不是每個情境都需要 store，如果網頁只是一個很簡單的一頁式活動頁面，還真的不需要。
...
Pinia 已設計得相當簡易了，除非你的情境真的很不適合 Pinia，不然一般情況還是乖乖用吧。<span class="text-nowrap">(´● ω ●`)</span>
...
我自己只有在「依賴方向明確、強耦合的元件」才會使用 Provide。
...
只要 `QForm` 內部含有使用 `useFormChild` 之元件，無論元件層級，提交時若有錯誤，都會自動阻擋，相當方便。

#### 酷酷元件的 card-futuristic

此[元件](https://chillcomponent.codlin.me/components/card-futuristic/)由多個子元件組成，子元件之間有強耦合關係。
...
如果資料只需在兩三個元件間共享，就不建議使用 store，而是由父元件提供資料。
...
但是這樣做一樣會遇到先前提到的問題（如何除錯、追蹤資料變更等等），所以我自己的習慣是：共享資料統一交給 store，Composable API 專注於組合、邏輯複用。
...
不過你可能會說：這樣要判斷 3 種情境，很麻煩欸！#(°口°〃)#

這時候可以使用 Vue 提供的 API：[`toValue`](https://vuejs.org/api/reactivity-utilities#tovalue)，輕鬆解決。
...
這個時候就可以使用 history API 的 state，但由於 Vue Router 已經占用了 state 資料，所以不能直接覆蓋，[要這樣寫](https://github.com/vuejs/vue-router/issues/2243#issuecomment-591439883)。
...
async function toProductDetail(product: Product) {
  router.push({
    ...
    state: {
      /**
       * state 不能傳遞複雜資料，clone 是為了轉為一般物件
       *
...
::: tip
[此功能正在 RFC 中](https://github.com/vuejs/rfcs/discussions/400)，未來可能新增 API 支援，大家可以關注關注。
:::

## 總結 🐟

- 元件優先只關心 Prop 來的資料
- 依賴方向明確、強耦合的元件使用 Provide
- 全域共享、範疇明確的資料才使用 store

以上是我自己的心得，大家如果有不同的想法，還請不吝告訴我。#(´▽`ʃ♡ƪ)#
