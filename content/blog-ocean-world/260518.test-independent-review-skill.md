...
三種模型對同一份 fixture 跑同樣 prompt，光看「能不能找到蟲」差異不大，但**誤報與嚴重度膨脹差很多**。%ԅ(´∀` ԅ)%
...
- **Case D 誤報**：Haiku 在 skill / 詳細都標 Critical，三種模型裡最愛找蟲湊字數；Opus 籠統也標了 2 個 Critical，是 production-grade 議題（onScopeDispose 缺失、timer 變數位置），但對小型 refactor 來說標太用力 %(´_ゝ`)%
- **Sonnet 4.6 最貼近標準答案**：Case D skill 視角 A 與籠統 prompt 都給出 0 Critical / 0 High，跟預期一致
- **Opus 4.7 High 膨脹明顯，且傾向把 production-grade 議題當 Critical**：對 Case D 的 unmount cleanup、SSR、delayMs 動態調整都標 High 或 Critical，其實是設計取捨而非 bug
...
Critical 持平，但 High 膨脹 skill 少 1/3（6 vs 9）。

原因是兩個 agent 各管一個視角，**每個 agent 不需要硬找超出自己範圍的問題**，膨脹傾向自然降低。%(*´ω`)人(´ω`*)%

### 3. skill 對「計畫類」審查特別關鍵

把籠統 prompt 跟 skill 在三個案例上拆開看：

| Case | Opus 籠統 | Sonnet 籠統 | skill 兩模型 |
| ---- | -------- | ----------- | ----------- |
| A 程式碼 | 7/9（-2） | 6/9（-3） | 都 9/9 |
| B 計畫 | **7/9（-2）** | **4/9（-5）** | 都 9/9 |
| C 規格 | 10/10（0） | 8/10（-2） | 都 10/10 |

**Sonnet 的 Case B 掉得最兇**，從 9/9 掉到 4/9。Opus 各 case 掉幅相近，但 Sonnet 在計畫類上明顯比程式碼跟規格不適應。

原因不難想，計畫類審查天然需要「可行性 + 排程」兩種視角交叉切入。

沒有正交視角分工的 prompt，代理人會在「該看可行性還是該看排程」之間反覆橫跳，最後兩邊都漏。

程式碼跟規格相對單一視角也能應付（A 看正確性、C 看一致性），所以受影響較小。

skill 的價值就是**強制把雙視角拆開、分給兩位代理人各自負責**，讓計畫類審查不再看模型強度看天吃飯 %( •̀ ω •́ )✧%

```text
路人：「為什麼計畫類特別需要分視角？」
鱈魚：「因為計畫不像程式碼有客觀對錯。可行性說『這樣寫沒問題』、排程說『但時間排錯了』，兩個都對，但要分開看才會完整 %( •̀ ω •́ )✧%」
```

skill 本身的價值就是上面這三條，沒有第四條 %(´∀`)%

不過實驗順帶挖出兩個跨模型副產物，跟 skill 設計也有點關連，一併記錄：

### 4. Case D 才是模型個性展現的舞台
...
- **Haiku**：對 unmount cleanup、SSR setTimeout、scope dispose 標 Critical，是真實 production 議題沒錯，但用在小型 refactor 上太用力 %(´_ゝ`)%
- **Sonnet**：唯一 Critical 出在「timer 跨 instance 共享」，標完又自己澄清「實務上不會發生」%(꒪ཀ꒪)%
- **Opus**：零 Critical 誤報，但 High 膨脹明顯，立場偏向「production-grade 應該處理」
...
**逐項清單反向誘發誤報**：Haiku 在 skill 標 2 個 Critical，籠統 prompt 反而 0 個（甚至寫「表現優異，無明顯缺陷」）。檢查清單變成「每項都要回應」的暗示，弱模型為了不空手就硬擠 Critical %(꒪ཀ꒪)%

**「Critical 須附觸發路徑」對弱模型沒約束力**：Opus 標 Critical 前先自驗觸發路徑，發現「customRef factory 只執行一次」就主動降為 High；Haiku 雖然會寫，但常寫得似是而非（例如把 Vue scope 已處理的失效 trigger 仍標為 memory leak）。

兩條合在一起就是：**強模型把 prompt 規則當約束，弱模型把它當填空題**。skill 文件該明確加上「逐項可標 pass，沒問題就 pass，不要為了填空而捏蟲」。
...
代理人類型可選 `Explore` 或 `general-purpose`。`Explore` 速度快、context 占用少，乍看很適合審查場景。

但 `Explore` 是定位搜尋型代理人，**只讀關鍵字命中片段，不會完整讀檔**，跨檔一致性、邊界條件這類深度任務會直接漏掉視窗外的內容 %(꒪ཀ꒪)%

審查仰賴完整脈絡，所以 skill 固定選 `general-purpose`。

雖然慢一點、貴一點，但避免「快速回應 + 漏看半個檔案」的隱性失誤 %( •̀ ω •́ )✧%

## 校準後的整合報告（示範 Case A）

把三種模型的 Case A skill 報告合併、嚴重度校準（採 Sonnet 4.6 的克制基線）、去除誤報後，Action List 與前一輪 Opus 單一模型實驗一致：
...
- **預埋命中是 prompt 結構的勝利，不是模型大小的勝利**：skill 與詳細 prompt 在三種模型都接近 100%，差距只在籠統 prompt
- **誤報才是模型能力的試金石**：Case D 把三種模型拉開，Opus 2C / 8H、Sonnet 1C / 4H、Haiku 3C / 3H
...
- **計畫類審查最仰賴正交視角**：Case B 在籠統 prompt 下 Sonnet 掉到 4/9，Opus 也只剩 7/9，相對 C 規格幾乎不受影響。skill 的價值就是把雙視角強制拆給兩位代理人，計畫類審查不再看模型強度看天吃飯
...
- 三種模型混合策略：Haiku 跑第一輪、Sonnet 整合、Opus 最後 review，看分工效果如何
