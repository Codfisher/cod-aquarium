...
- **prompt 結構決定命中率，模型大小決定誤報多寡**：skill / 詳細 prompt 在三種模型都接近 100%；籠統 prompt 下強模型撐得住、弱模型漏一堆。Case D 對照組就把差距拉開，Opus 2C / 8H、Sonnet 1C / 4H、Haiku 3C / 3H
- **計畫類審查最吃正交視角**：Case B 籠統下 Sonnet 掉到 4/9、Opus 也只剩 7/9，C 規格反而幾乎不受影響。skill 把雙視角強制拆開，計畫類就不用看模型強度看天吃飯
- **三種模型各有甜蜜點**：Haiku 快但需要人擋誤報，適合第一輪；Sonnet 嚴重度校準最貼近標準答案，整體最平衡；Opus 慢又貴，但每條 High 都站在正式環境角度，適合最後把關
- **skill 對弱模型是雙面刃**：逐項清單會讓 Haiku「為了不空手就標 Critical」，「Critical 須描述觸發路徑」這條規則對弱模型也只是形式，文件得明寫「沒問題就標 pass」
