...
本文延續概念篇內容，紀錄一些實作內容，大家若有更好的做法還請不吝指教。%(*´∀`)~♥%

本文技術堆疊以 Vue、NestJS 為例，相關套件版本如下：
...
::: tip
ts-rest 預計 `^3.53.0` 開始支援 [`Standard Schema`](https://github.com/standard-schema/standard-schema) 驗證，只要支援此規範的驗證器（例：Zod v4、[Valibot](https://valibot.dev/guides/introduction/)、[ArkType](https://arktype.io/)）的驗證器都可以用來定義合約資料，不限於 Zod 了！%✧⁑｡٩(ˊᗜˋ*)و✧⁕｡%
:::
...
可以看出 `age` 輸入形別是 `string`，但輸出形別是 `number`。
...
說到 Access Token，就一定會需要 Refresh Token 啦，讓我們加入 401 時自動 Refresh Token 邏輯：
...
這時候有個小問題，若有請求同時發出，就會出現重複 refresh 問題。

甚麼？你說你們後端不在意？那就跳...還是來加個基本處理好了。%(・∀・)９%

使用 Vue 的 `ref` 與 VueUse 的 [`until`](https://vueuse.org/shared/until/) 可以很簡單實現：
...
這樣可以處理基本上 9 成常見的情境了。%◝( •ω• )◟%

二次封裝後可以很輕鬆地加入各種自定義邏輯，有需求的話還可以加入 Queue 等等功能，這個就看實際需求而定了。
...
可以注意到 ts-rest 支援同時使用兩種 Client，這讓實務開發上更有彈性，可以讓專案保持簡單、乾淨，更不容易出現協作斷層。%( ‧ω‧)ノ╰(‧ω‧ )%

有些邏輯很單純的網頁使用一般的 Client 即可，有需要的部分再使用 Query Client。
...
ts-rest 回應的結果會將 HTTP Code 包含在資料內，其結構如下：
...
後端實作合約基本上就和概念篇相同，個人覺得實作上最重要的部分是可以拿合約寫 e2e 測試，這樣只要測試有過，API 內容就不會錯惹。%ԅ(´∀` ԅ)%

所以要怎麼拿合約寫 e2e 測試呢？

概念如下：

1. 將資料輸入合約，取得 REST API 資料（例如 url、query string、body data）
...
建立一個將 ts-rest 合約轉換成一般 REST API 資料的工具：
...
測試工具使用 Supertest，所以建立一個將 ts-rest 合約轉換後的 REST API 資料使用 Supertest 發送請求工具：
...
為了防止篇幅過長，這裡只展示部分測試內容，完整程式碼可以[在此取得](https://github.com/Codfisher/blog-ts-rest-practice/blob/main/apps/api-server/src/resource/collection-data/collection-data.e2e.spec.ts)。
