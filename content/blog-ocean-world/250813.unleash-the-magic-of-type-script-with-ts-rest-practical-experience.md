...
這時候有個小問題，若有請求同時發出，就會重複 refresh。
...
後端實作合約基本上就和概念篇相同，實作上最重要的部分是可以拿合約寫 e2e 測試，這樣只要測試有過，API 內容就不會錯惹。%ԅ(´∀` ԅ)%
...
測試工具使用 Supertest，建立一個將合約資料使用 Supertest 發送請求工具：
...
感謝大家看到這裡，以上為使用 ts-rest 實際開發的點點滴滴，或許會隨著時間推移而有所變化，所以本文會不定期更新。
