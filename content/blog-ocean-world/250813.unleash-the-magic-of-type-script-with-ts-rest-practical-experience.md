...
一般來說 GET API 會將資料放在 query string 傳輸，這樣會導致所有內容變成字串，其中數字與矩陣可能會出現歧異。

#### 數字與布林

URL 為 `/posts?take=10&draft=false`

伺服器會取得 `{ take: "10", draft: "false" }`，可以看到內容都是字串。

你可能會說：Zod 可以用 `z.coerce.number()`、`z.coerce.boolean()` 轉換啊？

這裡有個隱藏的小地雷，就是字串的 `"true"` 與 `"false"`，經過 `z.coerce.boolean()` 都會變成 `true`！%╭(°A ,°`)╮%
...
URL 為 `/posts?tags=js&tags=ts`

若 server 解析器支援陣列，會拿到 `['js', 'ts']`，這沒問題，只是若今天 `tags` 只有一個呢？
...
可以存在某個變數並 `export` 給其他檔案使用，不過實體會隨著微調 `options` 而持續增加，這次使用 memoization 方式處理。

::: tip 甚麼是 memoization
[先前的文章](/blog-program/250321.improve-speed-using-memoization)曾經聊過，有興趣的朋友可以來看看
:::

使用 `lodash-es` 提供的 `memoize` 調整一下：
