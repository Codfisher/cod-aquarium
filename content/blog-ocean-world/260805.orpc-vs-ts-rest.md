...
[oRPC](https://orpc.dev/) 全名為 OpenAPI Remote Procedure Call，同樣是實現端到端型別安全的 TypeScript 工具。
...
所謂「換一種協定」，其實就只是換掉 `new` 後面那個字而已。%(ﾟ∀ﾟ)%
...
以剛才的 `planet.find` 為例，端點就是 `POST /api/planet/find`，能跑，但 RPC 味很重。

想要像樣的 RESTful 端點，就在 procedure 補上 `.route()`。
...
- `RPCHandler`：oRPC 自家協定，可以傳 Date、File，但人類看不懂
- `OpenAPIHandler`：標準 REST，可以直接生 OpenAPI spec 丟給別人
...
代價是這條線只有 oRPC client 能通，你拿 cURL 會看到一堆天書。%(◜௰◝)%
...
只是這個寫法不算是正規的解法，因為波浪號是 oRPC 標記非公開 API 的慣例，不保證版本相容。

也有人也在[官方討論串](https://github.com/middleapi/orpc/discussions/1298)問了一模一樣的問題，作者本人的回覆就是這一行，還特別強調要從 contract 拿，不是從 client 拿。
...
`@Implement` 也可以一次吃下整包 router contract，底層會自動展開成一個個 NestJS method，套在原方法上的其他裝飾器也會跟著複製過去。

```ts
@Implement(accountContract)
account() {
  return {
    create: implement(accountContract.create).handler(async ({ input }) => {
      // ...
    }),
    find: implement(accountContract.find).handler(async ({ input }) => {
      // ...
    }),
  }
}
...
問題是 NestJS 的 `urlencoded` parser 不認得 Bracket Notation，而且 `application/json` 這類 content type 的檔案上傳，可能不會被解析成 `File`。
...
後端是 NestJS，e2e 得打真的 HTTP，所以要先把合約還原成 url、method、query、body 才交得給 Supertest。

實務篇為此刻了一個 `useContract`，接著每個資源還要再包一層 API 轉換層。

oRPC 合約本來就生得出 client，所以直接拿 client 打測試 server 就好，轉換層根本不用寫。
...
Nuxt 的 `.client.ts` 與 `.server.ts` 後綴會自動分流，所以 router 實作也不會被打包進瀏覽器，順便解決了「server 端程式碼外洩」這件事。
...
而且 oRPC 的生態系整合相當豐富，ts-rest 已停止更新，應該是不可能追上了。%( ˘･з･)%
