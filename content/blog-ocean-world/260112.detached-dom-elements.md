...
<<< ./detached-dom-elements/detached-elements.vue{14-18,37}
...
使用不同瀏覽器交叉測試後，發現某些瀏覽器外掛會導致問題（例如：密碼管理工具），關閉外掛後就沒問題了。
...
- 某些瀏覽器外掛會導致問題，關閉外掛後就沒問題了。
- 調整 `input` 出現的時機，避免大量建立、銷毀 `input` 元素。
